...
#pragma GCC diagnostic pop
# 15 "spmv_csr_adapative.cu"
template< class T> static void 
# 16
__wrapper__device_stub_spmv_csr_adaptive_kernel(T *&d_val, T *&d_vector, int *&d_cols, int *&d_ptr, int &N, int *&d_rowBlocks, T *&d_out) { ::cudaLaunchKernel(0, 0, 0, 0, 0, 0);}
#if 0
# 17
{ 
# 18
int startRow = d_rowBlocks[__device_builtin_variable_blockIdx.x]; 
# 19
int nextStartRow = d_rowBlocks[(__device_builtin_variable_blockIdx.x) + (1)]; 
# 20
int num_rows = nextStartRow - startRow; 
# 21
int i = __device_builtin_variable_threadIdx.x; 
# 22
__attribute__((unused)) static volatile T LDS[1024]; 
# 24
if (num_rows > 1) { 
# 25
...
__syncthreads(); 
# 36
for (int k = startRow + i; k < nextStartRow; k += (__device_builtin_variable_blockDim.x)) 
# 37
{ 
# 38
T temp = (0); 
...
T sum = (0); 
# 54
for (int j = rowStart + i; j < rowEnd; j += 1024) 
# 55
{ 
# 56
int col = d_cols[j]; 
...
__syncthreads(); 
# 64
for (int stride = (__device_builtin_variable_blockDim.x) >> 1; stride > 0; stride >>= 1) { 
# 65
__syncthreads(); 
# 66
if (i < stride) { 
...
# 16
spmv_csr_adaptive_kernel(T *d_val, T *d_vector, int *d_cols, int *d_ptr, int N, int *d_rowBlocks, T *d_out) 
# 17
{__wrapper__device_stub_spmv_csr_adaptive_kernel<T>(d_val,d_vector,d_cols,d_ptr,N,d_rowBlocks,d_out);
# 73
return;}
#if 0
# 17
...
# 1 "spmv_csr_adapative.cudafe1.stub.c"
#define _NV_ANON_NAMESPACE _GLOBAL__N__26_spmv_csr_adapative_cpp1_ii_87c1a48a
# 1 "spmv_csr_adapative.cudafe1.stub.c"
#include "spmv_csr_adapative.cudafe1.stub.c"
# 1 "spmv_csr_adapative.cudafe1.stub.c"
